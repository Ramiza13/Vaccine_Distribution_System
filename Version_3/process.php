...
<body>
 ...
 <footer class="p-3 mt-3 bg-teal text-white text-center">
        <div class="container">
           ...
           $q1 = oci_parse($conn, "begin check_num('".$phone."');end;");
           //$q1 = oci_parse($conn, "EXECUTE check_num('".$phone."')");

           oci_execute($q1);
           //echo $phone;

            $s2 = oci_parse($conn, "CREATE OR REPLACE procedure check_num(phn_no IN varchar)
            AS
            ...
            $s1 = oci_parse($conn, "select temp_val from temp");       
            oci_execute($s1);
            $row1 = oci_fetch_array($s1, OCI_NUM);
            
            if($row1[0])
            {
                echo "<h1>Enter 11 digits number.</h1>";
            }
            else{
